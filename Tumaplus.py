...
if "PARDE" in organisation or "SARCAF" in organisation:
    ...
    with col5b:
        styled_metric("Couple SASA", int(eyapfille_count))
    # c) Couple SASA
    eyap_garcon_count = (filtered_participation["Statut_group"] == 3.0).sum()
    with col5c:
        styled_metric("Couple SASA", int(eyap_garcon_count))
    # c) Couple SASA
    club_jeune_count = (filtered_participation["Statut_group"] == 4.0).sum()
    with col5d:
        styled_metric("Couple SASA", int(club_jeune_count))
...
if "ADJ" in organisation or "CARE" in organisation:
    ...
    # Mise en forme du graphique
    fig.update_layout(
        title="Progression des indicateurs de santé maternelle par période",
        xaxis_title="Période",
        yaxis_title="Nombre de cas",
...
if "ADJ" in organisation or "CARE" in organisation:
    ...
    # Mise en forme du graphique
    fig.update_layout(
        title="Évolution des décès liés à l'accouchement",
        xaxis_title="Période",
...
if "ADJ" in organisation or "CARE" in organisation:
    ...
    # Mise en forme du graphique
    fig.update_layout(
        title="Évolution des indicateurs CCC par période",
        xaxis_title="Période",
        yaxis_title="Valeurs",
...
st.header("INDICATEURS DU PROGRES")
st.subheader("En cours de traitement, ces indicateurs viseront a voir l'atteinte des resultats ")
